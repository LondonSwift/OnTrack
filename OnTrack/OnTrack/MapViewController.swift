...
class MapViewController: UIViewController {
  ...
  @IBOutlet weak var distanceLabelTopConstraint: NSLayoutConstraint!
  @IBOutlet weak var audableSwitchTopConstraint: NSLayoutConstraint!
  @IBOutlet weak var audableLabelTopConstraint: NSLayoutConstraint!
  @IBOutlet weak var audableDistanceLabel: UILabel!
  ...
  @IBAction func didChnageSlider(sender: UISlider) {
    ...
    defaults.setInteger(Int(slider.value), forKey:"OffTrackDistance")
  ...
  func updateSliderLabel() {
    ...
    let distance = defaults.integerForKey("OffTrackDistance")
 
      self.audableDistanceLabel.text = "(\(distance)m Audable)"
  ...
  func showFileList() {
    ...
    if let launchViewController = launchStoryboard.instantiateViewControllerWithIdentifier("FileListIdentifier") as? FileListTableViewController {
      ...
      self.view.pinView(launchViewController.view, inSuperView: self.fileListContainer, height:launchViewController.requiredHeight())
      ...
      self.fileListContainerHeightConstraint.constant = launchViewController.requiredHeight()
  ...
  func updateDistanceAndAudableDistanceLabelPositionsAndVisibility() {
    let defaults = NSUserDefaults.standardUserDefaults()
    
    let offTrackAudioOn = defaults.boolForKey("OffTrackAudioOn")
 
    self.distanceLabelTopConstraint.constant = offTrackAudioOn ? 14.0 : 18.0
    UIView.animateWithDuration(0.3) {
      self.view.layoutIfNeeded()
      
      self.audableDistanceLabel.alpha = offTrackAudioOn ? 1.0 : 0.0
  ...
  func showSettings() {
    ...
    self.updateDistanceAndAudableDistanceLabelPositionsAndVisibility()
    
    let defaults = NSUserDefaults.standardUserDefaults()
    
    let offTrackAudioOn = defaults.boolForKey("OffTrackAudioOn")
    
    self.settingsHeightConstraint.constant = offTrackAudioOn ? 150 : 110
    
    self.audableLabelTopConstraint.constant = offTrackAudioOn ? 114 : 74
    
    self.audableSwitchTopConstraint.constant = offTrackAudioOn ? 110 : 70
    ...
    UIView.animateWithDuration(0.3) {
      ...
      self.slider.alpha = offTrackAudioOn ? 1.0 : 0
    }
  ...
  func locationUpdated(location: CLLocation) {
    ...
    if self.repeater == nil {
      self.repeater = LSRepeater.repeater(15, execute: { [unowned self] () -> Void in
        ...
        self.updateDistanceLabel(minimumDistance)
  ...
  func updateDistanceLabel (distance:CLLocationDistance) {
    ...
    if distance < 1000 {
      self.distanceOffTrackLabel.text = String(format:"%.0fm", distance)
    }
    else {
      self.distanceOffTrackLabel.text = String(format:"%.1fkm", distance/1000)
    }
